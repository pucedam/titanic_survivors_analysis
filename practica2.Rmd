...
- Transofmramos el campo Survived como uno categórico con dos valores "Yes" y "Not", si el pasajero sobrevivió o no, respectivamente.
...
- Transformamos el campos Pclass como un ordinal ya que las clases tienen un orden y le damos los valores: "1st", "2nd", "3rd".
...
## 4.3.3 Análisis del modelo no supervisado jerárquico
...
maxAge <- max(ds$Age)
minAge <- min(ds$Age)
maxSibSp <- max(ds$SibSp)
minSibSp <- min(ds$SibSp)
maxParch <- max(ds$Parch)
minParch <- min(ds$Parch)
maxTicketOwners <- max(ds$TicketOwners)
minTicketOwners <- min(ds$TicketOwners)
maxPricePerPerson <- max(ds$PricePerPerson)
minPricePerPerson <- min(ds$PricePerPerson)

distance <- function (v1, v2) (as.integer(v1$Survived == v2$Survived) + as.integer(v1$Surname == v2$Surname) + as.integer(v1$Sex == v2$Sex) +  as.integer(v1$Embarked == v2$Embarked) + 1 - abs(as.integer(v1$Pclass) - as.integer(v2$Pclass)) / 2 + 1 - ((v1$Age - v2$Age) / (maxAge - minAge)) + 1 - ((v1$SibSp - v2$SibSp) / (maxSibSp - minSibSp)) + 1 - ((v1$Parch - v2$Parch) / (maxParch - minParch)) + 1 - ((v1$TicketOwners - v2$TicketOwners) / (maxTicketOwners - minTicketOwners)) + 1 - ((v1$PricePerPerson - v2$PricePerPerson) / (maxPricePerPerson - minPricePerPerson))) / 10
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
hc<-hclust(distance_matrix, method="ave")

plot(hc, hang = -1, labels = ds$Surivived)
rect.hclust(hc, k = 2)
...
distance <- function (v1, v2) (as.integer(v1$Survived == v2$Survived) + as.integer(v1$Sex == v2$Sex) + 1 - abs(as.integer(v1$Pclass) - as.integer(v2$Pclass)) / 2 + 1 - ((v1$Age - v2$Age) / (maxAge - minAge))) / 4
